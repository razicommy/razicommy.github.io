...
(function() {
    ...
    Knob.prototype.drawArcs = function(clickInteraction, dragBehavior) {
        ...
        if (this.options.displayInput) {
            ...
            svg.append("text").attr("id", "text").attr("text-anchor", "middle").attr("font-size", fontSize).style("fill", this.options.textColor).text( this.options.label || "").attr("transform", "translate(" + this.options.size / 2 + ", " + (this.options.size / 2 + this.options.size * .06) + ")");
        ...
        function interaction(x, y, isFinal) {
            ...
            if (that.value >= that.options.min && that.value <= that.options.max) {
                ...
                if (that.options.displayInput) {
                    ...
                    d3.select(that.element).select("#text").text(this.options.label || "");
                }
            }
        }
    };
    Knob.prototype.setValue = function(newValue) {
        if (!this.inDrag && this.value >= this.options.min && this.value <= this.options.max) {
            ...
            if (this.options.displayInput) {
                ...
                d3.select(this.element).select("#text").text(this.options.label || "");
            }
        }
    };
    ui.Knob = Knob;
